...
- ✅ **总测试数**: 85 个测试用例（v1.1.1 更新）
- ✅ **测试通过率**: 100%
- ✅ **覆盖模块**:
  - `useDocument`: 34 个测试（状态管理、文件操作、dirty 追踪、最近文件）
  - `fileService`: 14 个测试（文件选择、读写操作、错误处理）
  - `render/markdown`: 22 个测试（Markdown 渲染、安全消毒、**行号标记系统**）
  - `render/mermaid`: 11 个测试（Mermaid 渲染、并发控制、**错误隔离**）
  - `render/markdown-line-numbers`: 4 个测试（行号标记完整性验证）
